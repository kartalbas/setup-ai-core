...
| Skills and team modes | `rules/skills.md` says when to use them; `team-modes.tsv` says how each tool proves a mode is installed and how to install it; `session-start` refuses a session without them, `team-modes-install` installs them | skill folders in the project harness, deployed into every checkout |
...
| Prerequisite check and install | yes: `ai-core doctor` checks Git, gh and its login, Bash, PowerShell 7, Node.js 20+, jq, and reports the agent CLIs; installs missing required tools with winget, brew or apt-get; `init` runs it first and deploys nothing while a problem remains | tools a project harness declares |
...
│   ├── config.env                       yours: the agents served, Graft on or off, the organisation
│   ├── labels.tsv, assignees.tsv        yours: the label taxonomy and who owns which repository's issues
│   ├── team-modes.tsv                   yours: the team modes per tool, with probe and install command
...
`ai-core team-modes-install` runs the install column for every missing one. **Planned:** skills are
folders in the project harness, and `init` copies them into `.claude/skills/` and `.agents/skills/`
of every checkout.
...
| `team-modes.tsv` | tool, mode, level, probe, install, verified: how each tool proves a mode is installed and how it is installed (section 4.5). |
...
| `team-modes-check`, `team-modes-install` | section 4.5 |
