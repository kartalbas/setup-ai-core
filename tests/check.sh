...
echo "==> install puts the engine, the command and a working init into a home directory, both twins"
export AI_CORE_HOME="$WORK/home-sh/.ai-core"
bash "$ROOT/bin/install.sh" --engine "$ROOT" --no-path --no-doctor > "$WORK/install.sh.log" 2>&1 || fail "install.sh (see $WORK/install.sh.log)"
[ "$("$AI_CORE_HOME/bin/ai-core" version)" = "$(tr -d '\r\n' < "$ROOT/VERSION")" ] || fail "ai-core version"
"$AI_CORE_HOME/bin/ai-core" help > /dev/null || fail "ai-core help"
"$AI_CORE_HOME/bin/ai-core" no-such-command > /dev/null 2>&1 && fail "ai-core accepted an unknown command"
mkdir -p "$WORK/via-sh/.ai-core"; printf 'GRAFT_EXECUTION_MODE="skip"\n' > "$WORK/via-sh/.ai-core/config.env"
"$AI_CORE_HOME/bin/ai-core" init "$WORK/via-sh" --no-doctor > /dev/null 2>&1 || fail "ai-core init through the launcher"
(cd "$WORK/via-sh" && find . -type f | sort) | diff - "$WORK/sh.list" > /dev/null || fail "init through the launcher deployed a different file set"
bash "$ROOT/bin/install.sh" --engine "$ROOT" --no-path --no-doctor > /dev/null 2>&1 || fail "install.sh second run"
unset AI_CORE_HOME
export AI_CORE_HOME="$(native "$WORK/home-ps/.ai-core")"
pwsh -NoProfile -File "$ROOT/bin/install.ps1" -Engine "$(native "$ROOT")" -NoPath -NoDoctor > "$WORK/install.ps1.log" 2>&1 || fail "install.ps1 (see $WORK/install.ps1.log)"
[ "$(pwsh -NoProfile -File "$WORK/home-ps/.ai-core/bin/ai-core.ps1" version | tr -d '\r')" = "$(tr -d '\r\n' < "$ROOT/VERSION")" ] || fail "ai-core.ps1 version"
mkdir -p "$WORK/via-ps/.ai-core"; printf 'GRAFT_EXECUTION_MODE="skip"\n' > "$WORK/via-ps/.ai-core/config.env"
pwsh -NoProfile -File "$WORK/home-ps/.ai-core/bin/ai-core.ps1" init -TargetDir "$(native "$WORK/via-ps")" -NoDoctor > /dev/null 2>&1 || fail "ai-core.ps1 init through the launcher"
(cd "$WORK/via-ps" && find . -type f | sort) | diff - "$WORK/sh.list" > /dev/null || fail "init.ps1 through the launcher deployed a different file set"
unset AI_CORE_HOME
echo "  engine linked, command works, init through both launchers deploys the same files"
...
MARKERS="$(grep -c "^<!-- engine " "$WORK/sh/.ai-core/rules/rules.md")"
