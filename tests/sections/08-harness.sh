...
cmp -s "$ROOT/skeleton/docs/glossary.md" "$WORK/org-sh/shop-ai-core/docs/glossary.md" || fail "the created harness lacks the skeleton's glossary"
git -C "$WORK/org-sh/shop-ai-core" rm -q .gitattributes docs/glossary.md && git -C "$WORK/org-sh/shop-ai-core" -c user.name=check -c user.email=check@localhost commit -q -m 'without attributes' && git -C "$WORK/org-sh/shop-ai-core" push -q origin HEAD 2>/dev/null || fail "could not take .gitattributes and the glossary out of the harness clone"
...
grep -aq "note: example-org/shop-ai-core would get docs/glossary.md from the skeleton (dry run: not written)" "$WORK/layers-sh-attr-dry.log" || fail "init.sh --dry-run does not announce the skeleton's glossary (see $WORK/layers-sh-attr-dry.log)"
[ ! -e "$WORK/org-sh/shop-ai-core/.gitattributes" ] && [ ! -e "$WORK/org-sh/shop-ai-core/docs/glossary.md" ] || fail "init.sh --dry-run wrote into the harness clone"
...
grep -aq 'note: example-org/shop-ai-core: docs/glossary.md from the skeleton written into .*shop-ai-core; ai-core push commits it' "$WORK/layers-sh-attr.log" && cmp -s "$ROOT/skeleton/docs/glossary.md" "$WORK/org-sh/shop-ai-core/docs/glossary.md" || fail "init.sh did not give the older clone the skeleton's glossary (see $WORK/layers-sh-attr.log)"
git -C "$WORK/org-sh/shop-ai-core" add .gitattributes docs/glossary.md && git -C "$WORK/org-sh/shop-ai-core" -c user.name=check -c user.email=check@localhost commit -q -m 'attributes again' && git -C "$WORK/org-sh/shop-ai-core" push -q origin HEAD 2>/dev/null || fail "could not commit .gitattributes and the glossary back into the harness clone"
...
cmp -s "$ROOT/skeleton/docs/glossary.md" "$WORK/org-ps/store-ai-core/docs/glossary.md" || fail "the harness init.ps1 created lacks the skeleton's glossary"
git -C "$WORK/org-ps/store-ai-core" rm -q .gitattributes docs/glossary.md && git -C "$WORK/org-ps/store-ai-core" -c user.name=check -c user.email=check@localhost commit -q -m 'without attributes' && git -C "$WORK/org-ps/store-ai-core" push -q origin HEAD 2>/dev/null || fail "could not take .gitattributes and the glossary out of store-ai-core"
...
grep -aq 'note: example-org/store-ai-core: docs/glossary.md from the skeleton written into .*store-ai-core; ai-core push commits it' "$WORK/layers-ps-attr.log" && cmp -s "$ROOT/skeleton/docs/glossary.md" "$WORK/org-ps/store-ai-core/docs/glossary.md" || fail "init.ps1 did not give the older clone the skeleton's glossary (see $WORK/layers-ps-attr.log)"
git -C "$WORK/org-ps/store-ai-core" add .gitattributes docs/glossary.md && git -C "$WORK/org-ps/store-ai-core" -c user.name=check -c user.email=check@localhost commit -q -m 'attributes again' && git -C "$WORK/org-ps/store-ai-core" push -q origin HEAD 2>/dev/null || fail "could not commit .gitattributes and the glossary back into store-ai-core"
